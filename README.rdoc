...
Assuming you have a class which implements .parse and .unparse methods, you
can hook it in like this:

  db = CouchTiny::Database.url("http://127.0.0.1:5984/testdb",
         :parser => CouchTiny::JSObjectParser)
  doc = db.get("12345")
  puts doc.foo

Here, JSObjectParser makes hashes behave like Javascript objects - you can
access them as doc.foo or doc['foo'] equally.

...
callbacks or validation - the latter is probably best done in a
validate_doc_update function (**). But it does allow you to associate a
document with a database and save it back to that database; to create
classes of documents; to associate those classes with a definable 'type'
attribute; and to instantiate objects from views.

...
converts exceptions back to the usual Net::HTTP hierarchy and parses the
JSON error message. This is easily replaced if you want.
...
=== CouchTiny::Document
  ...
  class Foo < CouchTiny::Document
    use_database "http://127.0.0.1:5984/foo"
...
ActiveSupport 2.3.2 and 2.3.2.1 break JSON badly. If you get the following
error:
...
then either try using Rails HEAD, or make sure that you require
active_support before you require couchtiny or json. To test using
script/console:
